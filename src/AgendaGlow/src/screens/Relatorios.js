...
export default function Relatorios() {
  ...
  const onChangeDate = (event, selectedDate) => {
    ...
    if (selectedDate) {
      ...
      if (selectedDate > hoje) {
        Alert.alert(
          "Data Inválida",
          "O relatório não pode filtrar datas futuras."
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    const unsubS = listenServicos((lista) => {
      setServicosList(
        (lista || []).map((s) => ({ id: s.id || s.sid, nome: s.nome || s }))
      );
    });
    const unsubC = listenClientes((lista) => {
      setClientesList(
        (lista || []).map((c) => ({ id: c.id || c.cid, nome: c.nome || c }))
      );
    });
  ...
  useEffect(() => {
    ...
    if (filters.servico.length > 0) {
      lista = lista.filter((a) => {
        ...
        const itemIds = items.map((i) =>
          typeof i === "object" && i !== null ? i.id || i.sid : i
        );
    ...
    if (filters.profissional.length > 0) {
      lista = lista.filter((a) => {
        ...
        const itemIds = items.map((i) =>
          typeof i === "object" && i !== null ? i.id || i.uid : i
        );
  ...
  const gerarPDF = async () => {
    try {
      await generateRelatoriosPDF(filteredRelatorios, filters, {
        getClienteNome,
        getServicoNome,
        getFuncionarioNome,
      });
